...
const listCommand = buildCommand({
  docs: { brief: "List bounced email addresses" },
  parameters: {
    flags: {
      email: {
        kind: "parsed",
  ...
  async func(this: void, flags: ListFlags) {
    ...
    try {
      const results = await manager.runAcrossAccounts(
        (client) => client.getBounces({ email: flags.email, limit: flags.limit }),
        flags.account,
      );
      if (flags.json) {
...
const deleteCommand = buildCommand({
  docs: { brief: "Delete a bounce by email address" },
  parameters: {
    flags: {
      account: {
        kind: "parsed",
    ...
    positional: {
      kind: "tuple",
      parameters: [{ brief: "Email address to remove from bounce list", parse: String }],
    },
  },
  async func(this: void, flags: { account: string | undefined }, email: string) {
    ...
    try {
      const results = await manager.runAcrossAccounts(
        (client) => client.deleteBounce(email),
        flags.account,
      );
      for (const r of results) {
        if (r.error) {
          ...
          console.log(`[${r.account}] Deleted bounce for ${email}`);
        }
      }
    } catch (err) {
      console.error(`Error: ${err instanceof Error ? err.message : err}`);
...
export const bouncesRoutes = buildRouteMap({
  routes: { list: listCommand, delete: deleteCommand },
  docs: { brief: "Manage bounced email addresses" },
});
